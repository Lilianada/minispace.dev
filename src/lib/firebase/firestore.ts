...
const formatDoc = (doc: QueryDocumentSnapshot<DocumentData> | DocumentSnapshot<DocumentData>, userId?: string) => {
  if (!doc.exists()) {
    return null;
  }
  ...
  posts: {
    ...
    create: async (userId: string, data: {
      ...
      try {
        ...
        const { title, content, excerpt, status = 'draft', tags = [] } = data;
        ...
        console.log('Creating post for user:', userId, 'with data:', { ...postData, createdAt: 'timestamp', updatedAt: 'timestamp' });
    ...
    // Update a post
    update: async (userId: string, postId: string, data: {
      title?: string;
      content?: string;
      excerpt?: string;
      status?: 'published' | 'draft';
    ...
    updateStatus: async (userId: string, postId: string, status: 'published' | 'draft') => {
      try {
